...
reactive_lfs_last_year <- reactive({
  lfs_data|>
    ...
    bind_rows(lfs_data)|>
    filter(industry==rev(rownames(temp))[input$point2$y])|>
    mutate(alpha=if_else(series=="12 month rolling mean", 1, .25))
})
...
plotly::renderPlotly({
  ...
  plt <- ggplot()+
    geom_line(data=reactive_tbbl2(), 
              mapping= aes(date, value, colour=series))+
    geom_line(data=reactive_lfs_last_year(), 
              mapping= aes(date, value, colour=series, alpha=alpha))+
  ...
  plt <- region_shares|>
    ...
    ggplot(aes(year, 
               ...
               text=paste0("year: ",
                           ...
                           "\n value: ",
                           if(input$what=="count"){
                             scales::comma(get(input$what), accuracy = 1)
                           }else{
                             scales::percent(get(input$what), accuracy = .1)
                           },
                           "\n CAGR: ",
                           if(input$what=="count"){
                             scales::percent(count_cagr, accuracy = .1)
                           }else{
                             scales::percent(share_cagr, accuracy = .1)
                           }
                           )
               )
           )+
    ...
    ggplot(aes(year, 
               ...
               text=paste0("year: ",
                           ...
                           if(input$what2=="count"){
                             scales::comma(get(input$what2), accuracy = 1)
                           }else{
