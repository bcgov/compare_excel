...
reactive_tbbl <- reactive({
   reactive_base()|>
    ...
    select(year, variable, percent_change)|>
...
temp <- internal_vs_stokes|>
  ungroup()|>
  select(industry, date, percent_change)|>
  filter(industry!="Total")|>
  distinct()|>
  pivot_wider(id_cols = industry, names_from = "date", values_from = "percent_change")|>
  column_to_rownames("industry")

  heatmaply::heatmaply_cor(temp, dendrogram=FALSE, key.title="percent change", showticklabels = c(FALSE, FALSE))|>
  onRender(js2)
```
