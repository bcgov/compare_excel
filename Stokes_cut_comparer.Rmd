...
min_abs_original <- 10
#functions-----------------------
set_svg_download <- function(plt) {
  plotly::config(p, toImageButtonOptions = list(format = "svg"))
}
...
reactive_tbbl <- reactive({
   tbbl <- reactive_base()|>
    ...
    row_order <- order(matrixStats::rowMaxs(tbbl))  
    tbbl[row_order, ]
})
...
reactive_wide <- reactive({
  ...
  row_order <- order(matrixStats::rowMaxs(wide))  
  wide[row_order, ]
})
